...
7. Obtain `Secret` from `Dbay->Clients->dbay->Credentials`. Create a file `.env` under directory `dbay/backend`. Write `CLIENT_SECRET=<secret-you-just-obtained>`, newline, and `SESSION_SECRET=<string-that-you-set>`. Keep the session secret consistent through runs.
8. Open two terminal windows and goto `backend` and `frontend` correspondingly. For each terminal, run `npm i`. Then start the backend first, then the frontend.
9. 
## Getting started
...
```
cd existing_repo
git remote add origin https://coursework.cs.duke.edu/by73/dbay.git
git branch -M main
...
- [ ] [Set up project integrations](https://coursework.cs.duke.edu/by73/dbay/-/settings/integrations)
